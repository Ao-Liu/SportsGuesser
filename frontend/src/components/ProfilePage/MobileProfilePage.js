...
const MobileProfilePage = () => {
  ...
  const typoStyleIntro = {
    ...
    fontSize: '3vw' // Increased font size
  };

  const GamesRecord = {
    ...textStyles,
    marginTop: '1vw',
    fontSize: '4vw' // Increased font size
  };
  ...
  return (
    <div style={containerStyle}>
      <Box textAlign="center" mb={2}>
        <Avatar alt={userData.userName} src={userData.photoUrl} style={{ width: '30vw', height: '30vw', ...avatarStyle }} />
      </Box>
      <Typography style={{ ...textStyles, fontSize: '6vw' }} variant="h2">
      ...
      <Box display="flex" justifyContent="center">
        {userData.conqueredCourNameUrl.map((court, index) => (
          <div key={index} style={{  marginRight: '1.5vw', marginRight: '1.5vw', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Avatar alt={court.name} src={court.url} style={{ width: '30vw', height: '30vw', ...avatarStyle, marginTop: '5vw' }} />
            <Typography style={{ ...textStyles, fontSize: '3vw' }} variant="subtitle1">
              {court.name}
            </Typography>
          </div>
        ))}
